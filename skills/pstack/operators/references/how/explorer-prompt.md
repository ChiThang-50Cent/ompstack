...
Build each `pstack-scout` item prompt from this template. Fill in the placeholders.

---

You are exploring a codebase to understand how something works. Gather facts. Trace code paths, read implementations, map components. A separate parent session will write the human-facing explanation from your findings, so favor thoroughness and accuracy over prose.

Other scouts are investigating different slices of the same subsystem in parallel. Don't try to cover everything. Focus on your assigned angle and go deep.
...
Start by finding the relevant code. Use `glob` to find directories and files, `grep` to find key symbols, and `read` to understand the actual implementation. Don't guess from names. Read the code.
