...
export function normalizeRouteInput(input) {
  if (!plainObject(input) || Object.hasOwn(input, "measurementPurpose") || !ROUTE_INTENTS.includes(input.intent) || !Array.isArray(input.targets) || input.targets.length === 0 || !input.targets.every(normalizedPath) || new Set(input.targets).size !== input.targets.length || !exactKeys(input.taskFacts, ["behaviorAffecting"]) || typeof input.taskFacts.behaviorAffecting !== "boolean" || !plainObject(input.repository) || !isAbsolute(input.repository.root) || !["base", "head"].every((key) => typeof input.repository[key] === "string" && input.repository[key] !== "") || !plainObject(input.riskFacts) || typeof input.riskFacts.sharedSemanticBoundary !== "boolean" || !Number.isInteger(input.riskFacts.consumerFamilies) || input.riskFacts.consumerFamilies < 0 || !Number.isInteger(input.riskFacts.executionModes) || input.riskFacts.executionModes < 0 || typeof input.riskFacts.graphTraversal !== "boolean" || typeof input.riskFacts.materialUnknown !== "boolean" || !plainObject(input.graphPolicy) || !validSourceRoots(input.graphPolicy.sourceRoots)) fail("has an invalid shape");
  ...
  const value = Object.freeze({ ...input, targets: Object.freeze([...input.targets].sort()), taskFacts: Object.freeze({ ...input.taskFacts }), repository: Object.freeze({ ...input.repository }), riskFacts: Object.freeze({ ...input.riskFacts }), graphPolicy: Object.freeze({ ...input.graphPolicy, sourceRoots }) });
