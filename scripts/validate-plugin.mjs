...
assert.match(skill, /`taskFacts\.behaviorAffecting` declares whether the change alters observable product behavior/);
assert.match(skill, /`riskFacts\.sharedSemanticBoundary` declares whether the change touches a contract/);
assert.match(skill, /`riskFacts\.consumerFamilies` counts independent caller families/);
assert.match(skill, /`riskFacts\.executionModes` counts affected execution modes/);
assert.match(skill, /`riskFacts\.graphTraversal` declares whether the change modifies graph traversal/);
assert.match(skill, /`riskFacts\.materialUnknown` is `true` when the changed surface was not directly inspected/);
assert.match(skill, /`consumerFamilies: 0` and `executionModes: 0` mean unknown and force High/);
assert.match(skill, /Conservative declarations are not free\./);
