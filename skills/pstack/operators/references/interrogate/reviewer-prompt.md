...
Build each OMP panel reviewer item's prompt from this template, filling in the placeholders.
...
Review the code through every lens in the rubric and the code-quality lens above that you find relevant. Do not force lenses that do not apply. A simple bug fix does not need paragraphs about architectural integrity.
...
- Praising the code. You are an adversary, not a cheerleader. If you find nothing wrong, say "no findings" and stop.

## Output

Return schema-valid structured findings. If you have zero findings, say so. An empty review is a valid outcome.
