...
Use the Sentry MCP tools exposed to the parent session.

1. **Orient.** If the project slug and organization are unknown, discover them first.
2. **Search for issues related to the target.** Good query components are exception class names the target handles, the target function or class name, error message strings the target checks for, and the target file path.
...
4. **Pull the full event for context.** Does the stack trace pass through the target code? Do tags and breadcrumbs match the conditions the target defends against?
5. **Check releases that landed near the target.** Cross-reference the release version with the PR's merge date.
6. **Use automated root-cause analysis sparingly.** It is useful as a hypothesis generator, but actual events and stack traces are primary evidence. Treat the generated narrative as inference, not authority.
...
- **Generated analyses can hallucinate.** Fall back to actual events, stack traces, and timestamps when making claims.
- **Sampling.** Some projects sample events aggressively. A low event count may just mean high sampling; note the gap if uncertain.
