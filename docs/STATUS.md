...
**Snapshot:** `v0.3.7` (`2026-09-18`). Update this page in every release with shipped behavior, current evidence, and unresolved work.
...
On the 13 supported-language security PRs selected from AACR-Bench commit `68a5697` (0 commit-unavailable and 0 base-validation exclusions), the tiered generator raised M3 from **2/13** on the pre-change overlay to **3/13**. The final M3 split was Go 2/6, TypeScript 1/4, Java 0/2, and Python 0/1. This is a positive-security-only slice, not a clean-PR precision estimate.
