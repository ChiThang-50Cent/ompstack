...
/** Classifies measured routing inputs; unknown source evidence can never retain Low or Medium. */
export function classifyRoute({ signals, graph, taskFacts, riskFacts, policy }) {
  ...
  const elevated = reasons.some((reason) => !reason.startsWith("threshold:"));
  const thresholdExceeded = reasons.some((reason) => reason.startsWith("threshold:"));
  const risk = critical.length ? "critical" : elevated ? "high" : taskFacts.behaviorAffecting || thresholdExceeded ? "medium" : "low";
