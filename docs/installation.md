...
Persisted session state is versioned. Version 0.5.0 writes state version 2 (`pstack-omp/state-v2`); version 1 state from 0.1.0 is not restored. A future incompatible state migration must increment that version and provide explicit restoration logic rather than silently interpreting old state.
