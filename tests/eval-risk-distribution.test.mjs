...
test("risk distribution evaluator records the selected external repository", async () => {
  ...
  try {
    ...
    const process = Bun.spawn([Bun.which("bun"), fileURLToPath(new URL("../scripts/eval-risk-distribution.mjs", import.meta.url)), "--repo", root, "--count", "2"], { stdout: "pipe", stderr: "pipe" });
    const [stdout, stderr, exitCode] = await Promise.all([new Response(process.stdout).text(), new Response(process.stderr).text(), process.exited]);
    assert.equal(exitCode, 0, stderr);
