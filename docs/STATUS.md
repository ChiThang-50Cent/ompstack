...
- Test suite at this snapshot: **116 passing tests across 19 files**.
- `bun scripts/session-telemetry.mjs --session <path/to/session.jsonl> [--json]` correlates native OMP tool lifecycle entries with Ompstack RouteDecision, block, tool, and evidence-attempt entries. It reports outcomes and bounded `agent://`, `artifact://`, and `history://` references; it does not promote a launched task into accepted proof.
